...
namespace MySTL
{
	...
	public:
		...
		vector(const std::size_t n, const T& v);						// constructor: fill
		vector(initializer_list<T> il);									// constructor: initializer_list
		vector(const vector &s);										// constructor: copy
		vector(vector &&v) noexcept;									// constructor: move 
		template<typename InputIterator> 
		vector(InputIterator first, InputIterator second);				// constructor: range

		vector& operator=(const vector &v);								// assign content: copy
		vector& operator=(initializer_list<T> il);						// assign content: initializer list
		vector& operator=(vector &&v);									// assign content: move
		~vector();                                                      // destructor
		...
		void assign(initializer_list<T> il);							// assign vector content: initializer list
		...
		T* insert(const T* position, initializer_list<T> il);		// insert elements: initializer list
